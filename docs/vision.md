...
## 1. The "Why"
We are building this library because existing agent frameworks (LangChain, CrewAI, etc.) suffer from two extremes:
1.  **Too Abstract:** They hide the control flow behind "Chains" and "Executors," making it impossible to debug or optimize the agent's decision loop.
2.  **Too Rigid:** They enforce specific architectures (e.g., "Role-Based" or "Graph-Based") that constrain research and experimentation.

We need a **thin, transparent wrapper** that provides the *essential primitives* for building agents without imposing a specific architecture. Our goal is to enable **researchers and developers** to:
- Rapidly prototype standard agents (Chatbots, RAG).
- Easily experiment with novel architectures (Deep Agents, recursive planning, self-optimizing swarms).
...
In our library, an `Agent` is not a black box. It is a **Controller** that implements a specific loop. 
...
*   This enables infinite nesting and specialization without complex "Graph" definitions.
...
2.  **Production**: Deploying a customer service bot. (Use the standard `SingleAgent` with `RedisStorage`).
3.  **Complex Tasks**: Building a "Software Engineer" agent that plans, writes, tests, and fixes code using sub-agents.

## 6. What We Are NOT
*   **NOT LangChain**: We do not have "Chains" or hidden prompt templates.
