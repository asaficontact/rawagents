...
class AsyncLLM:
    ...
    def __init__(
        ...
        *,
        ...
        self._instructor = instructor.from_litellm(acompletion)
    ...
    async def complete_structured(
        ...
        *,
        ...
        """Completion with Pydantic structured output (async).
        ...
        Returns:
            The parsed Pydantic model instance, or tuple if include_metadata=True.
        """
        model = model or self.config.model
        ...
        # Non-metadata path
        parsed: T = await self._instructor.chat.completions.create(**instructor_kwargs)
        return parsed
    ...
    async def complete_with_tools(
        ...
        *,
        ...
        """Completion with tool/function calling (async).

        IMPORTANT: This method returns tool call REQUESTS but does NOT execute them.

        Args:
            messages: List of message dicts.
            tools: List of Pydantic model classes defining available tools.
            model: Model identifier. Uses default if not provided.
        ...
        Returns:
            ToolResponse with tool_calls list.
        """
        model = model or self.config.model
    ...
    async def stream(
        ...
        *,
        ...
        """Stream text completion (async).

        Returns an async iterator that yields text chunks as they arrive.

        NOTE: Streaming does NOT support tools, structured output, or reasoning in v1.0.
